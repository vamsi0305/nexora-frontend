...
import { MessageCreatorModal } from "@/components/ideas/MessageCreatorModal";
import { Badge } from "@/components/ui/Badge";
import { Button } from "@/components/ui/Button";
import { LoadingSkeleton } from "@/components/shared/LoadingSkeleton";
import { Share2, Info, Calendar } from "lucide-react";
import Link from "next/link";

export default function IdeaDetailPage() {
  const { id } = useParams() as { id: string };
  const { fetchIdeaById, ideaDetail: idea, loading, getSupportPledges } = useIdeas();
  ...
  useEffect(() => {
    ...
    getSupportPledges(id).then((pledges) => {
      setIsSupporter(
        pledges.some((pledge) => pledge.user_id === user.id && pledge.status !== "rejected")
      );
    });
  }, [id, user, getSupportPledges]);
        ...
        <div className="glass p-6 rounded-2xl flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-white/10">
            ...
            {isSupporter && user?.id !== idea.user_id && (
              <MessageCreatorModal ideaId={idea.id} creatorName={idea.users?.name} />
            )}
